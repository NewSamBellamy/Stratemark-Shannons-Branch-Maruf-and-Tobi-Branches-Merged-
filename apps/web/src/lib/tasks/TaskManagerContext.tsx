...
interface TaskManagerContextType {
  ...
  setMarketUpdates: (taskId: string, updates: MarketUpdate[]) => void;
  completeTask: (
    taskId: string,
...
export function TaskManagerProvider({ children }: { children: ReactNode }) {
  ...
  const setMarketUpdates = useCallback((taskId: string, updates: MarketUpdate[]) => {
    setTasks((prev) => prev.map((task) => (task.id === taskId ? { ...task, marketUpdates: updates } : task)));
  }, []);
