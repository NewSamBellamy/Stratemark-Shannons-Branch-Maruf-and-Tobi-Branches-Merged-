  ...
  return (
    <div className="relative" ref={panelRef}>
      <button
        ...
        className={cn(
          'relative inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-medium transition-colors',
          activeCount > 0
            ? 'border-primary/40 bg-primary/10 text-primary-ink hover:bg-primary/15'
            : 'border-border bg-surface text-muted hover:bg-surface-2 hover:text-content',
        )}
...
function TaskDrawer({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div
      role="dialog"
      aria-label="Research and task monitor"
      className="fixed inset-x-3 top-16 z-50 max-h-[calc(100dvh-5rem)] overflow-hidden rounded-2xl border border-border bg-surface shadow-card sm:absolute sm:inset-x-auto sm:right-0 sm:top-11 sm:w-96"
    >
      {/* Drawer Header Bar: monitor title & cleanup action */}
      ...
      <div className="max-h-[calc(100dvh-9.5rem)] space-y-2 overflow-y-auto p-3 sm:max-h-96">
        {tasks.length === 0 ? (
          <p className="py-6 text-center text-xs text-muted">
            No recent tasks or research activity.
          </p>
        ) : (
          tasks.map((task) => (
            <TaskItem
              ...
              onToggleLog={() => setExpandedLogId((prev) => (prev === task.id ? null : task.id))}
    ...
    <div className="panel-2 p-3 text-xs">
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        >
          ...
          {isCompleted && <CheckCircle2 className="h-4 w-4 shrink-0 text-positive" />}
      ...
      {isCompleted && (
        <p className="mt-1.5 text-[11px] text-positive">
          Ready to review{task.currentStep ? ` · ${task.currentStep}` : ''}
        </p>
      )}
      ...
      <div className="mt-2.5 flex flex-wrap items-center justify-between gap-2 border-t border-border pt-2 text-[11px]">
        <button
          type="button"
          onClick={onToggleLog}
      ...
      {expanded && (
        <div className="research-terminal mt-2 max-h-40 overflow-y-auto rounded-lg p-2 font-mono text-[11px] leading-relaxed space-y-1">
