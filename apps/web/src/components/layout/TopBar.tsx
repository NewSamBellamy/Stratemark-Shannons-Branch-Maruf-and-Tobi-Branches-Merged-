...
export function TopBar() {
  ...
  return (
    <div className="relative z-30 flex min-w-0 flex-1 items-center justify-between gap-4">
      <div className="hidden min-w-0 flex-1 truncate text-sm text-muted lg:block">Competitive intelligence, card by card</div>
      <div className="flex min-w-0 flex-wrap items-center justify-end gap-2">
        {isDemoMode && (
          <button
            onClick={() => openUpgradeModal('Upgrade to Pro for unlimited AI market research queries.')}
            className="chip max-w-[260px] cursor-pointer border-neutral/40 bg-neutral/10 text-neutral hover:bg-neutral/20"
        ...
        {error && (
          <div
            className="flex cursor-pointer items-center gap-1.5 rounded border border-negative/40 bg-negative/10 px-2 py-1 text-xs text-negative"
            onClick={clearError}
            title={error}
          >
        ...
        {isAuthenticated && user ? (
          <div className="relative" ref={dropdownRef}>
            ...
            {dropdownOpen && (
              <div className="mi-modal-in absolute right-0 z-50 mt-2 w-64 rounded-xl border border-border bg-surface p-3 shadow-card">
                <div className="flex items-center gap-3 border-b border-border pb-3">
                  ...
                  <div className="overflow-hidden">
                    ...
                    <span className="mt-1 inline-flex items-center gap-1 rounded border border-positive/40 bg-positive/10 px-1.5 py-0.5 text-[10px] font-medium text-positive">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => openUpgradeModal('Upgrade to Pro for unlimited AI market research queries.')}
              className="btn-primary cursor-pointer px-3 py-1.5 text-xs font-semibold"
            >
