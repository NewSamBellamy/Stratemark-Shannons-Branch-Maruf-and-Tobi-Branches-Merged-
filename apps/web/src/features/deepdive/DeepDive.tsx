...
type PanelSize = 'default' | 'wide' | 'full' | 'pip';
...
export function DeepDiveProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <DeepDiveContext.Provider
      value={{ open: noop, chat: noop, openThread: noop, closePanel: noop, isOpen: false }}
    >
      {children}
    </DeepDiveContext.Provider>
  );
...
export function DeepDiveProviderWithPanel({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    panelRef.current?.toggleAttribute('inert', !openState);
  }, [openState]);
  useEffect(() => {
  ...
  const panelIsPip = panelSize === 'pip';
  const panelSizeClass =
    panelSize === 'full'
      ? 'w-full'
      : panelSize === 'wide'
        ? 'w-full sm:w-[min(50vw,720px)]'
        : panelIsPip
          ? 'h-[min(72vh,640px)] w-[min(calc(100vw-2rem),440px)]'
          : 'w-full sm:w-[400px]';
  const panelPlacementClass = panelIsPip
    ? panelSide === 'left'
      ? 'bottom-4 left-4 right-auto top-auto rounded-2xl border shadow-2xl'
      : 'bottom-4 left-auto right-4 top-auto rounded-2xl border shadow-2xl'
    : panelSide === 'left'
      ? 'left-0 right-auto top-0 h-full border-y-0 border-l-0 border-r'
      : 'left-auto right-0 top-0 h-full border-y-0 border-l border-r-0';
  const panelHiddenTransform = panelSide === 'left' ? '-translate-x-full' : 'translate-x-full';

  return (
    <DeepDiveContext.Provider
      value={{ open, chat, openThread, closePanel: close, isOpen: openState }}
    >
      {children}

      {/* AI research panel. AppShell adjusts the main area for the default right dock. */}
      <aside
        ref={panelRef}
        id="ai-research-panel"
        className={cn(
          'fixed z-40 flex flex-col border-border bg-surface transition-[transform,width,height,top,right,bottom,left] duration-300 ease-out',
          panelSizeClass,
          panelPlacementClass,
          openState ? 'translate-x-0' : panelHiddenTransform,
        )}
        role="dialog"
          ...
          <div className="min-w-0">
            ...
            >
              ...
              <button
                ...
                className={cn(
                  'rounded-md px-1.5 py-1 text-[10px] text-muted hover:bg-surface-2 hover:text-content',
                  panelSize === 'default' && 'bg-surface-2 text-content',
                )}
                aria-label="Use default panel width"
                aria-pressed={panelSize === 'default'}
                title="Default width"
              >
                Default
              </button>
              <button
                ...
                className={cn(
                  'inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-[10px] text-muted hover:bg-surface-2 hover:text-content',
                  panelSize === 'wide' && 'bg-surface-2 text-content',
                )}
                aria-label="Use wider panel, up to half the screen"
                aria-pressed={panelSize === 'wide'}
                title="Wider panel (up to half screen)"
              >
                <PanelRightOpen className="h-3 w-3" /> Wide
              </button>
              <button
                type="button"
                ...
                className={cn(
                  'inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-[10px] text-muted hover:bg-surface-2 hover:text-content',
                  panelSize === 'full' && 'bg-surface-2 text-content',
                )}
                aria-label="Use full-screen panel"
              ...
              <button
                type="button"
                onClick={() => setPanelSize('pip')}
                className={cn(
                  'inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-[10px] text-muted hover:bg-surface-2 hover:text-content',
                  panelIsPip && 'bg-surface-2 text-content',
                )}
                aria-label="Use small floating picture-in-picture panel"
                aria-pressed={panelIsPip}
                title="Small floating panel"
              >
              ...
              <button
                ...
                className={cn(
                  'inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-[10px] text-muted hover:bg-surface-2 hover:text-content',
                  panelSide === 'left' && 'bg-surface-2 text-content',
                )}
                aria-label="Dock panel on the left"
                aria-pressed={panelSide === 'left'}
                title="Dock left"
              >
                <PanelLeft className="h-3 w-3" /> Left
              </button>
              <button
                type="button"
                ...
                className={cn(
                  'inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-[10px] text-muted hover:bg-surface-2 hover:text-content',
                  panelSide === 'right' && 'bg-surface-2 text-content',
                )}
                aria-label="Dock panel on the right"
                aria-pressed={panelSide === 'right'}
                title="Dock right"
              >
                <PanelRight className="h-3 w-3" /> Right
              </button>
            </div>
          </div>
