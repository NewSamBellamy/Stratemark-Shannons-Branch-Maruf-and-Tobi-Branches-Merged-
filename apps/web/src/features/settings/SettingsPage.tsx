    ...
    <div className="mx-auto max-w-2xl">
      ...
      <div className="panel mt-6 space-y-4 p-6">
        <div className="flex items-center gap-2">
          ...
          {hasKey && (
            <span className="chip border-positive/40 bg-positive/10 text-positive">
        ...
        <div>
          ...
          {draft.length > 0 && !looksLikeGeminiKey(draft) && (
            <p className="mt-2 flex items-start gap-1.5 text-xs text-neutral">
        ...
        {test.status !== 'idle' && (
          <div
            className={
              test.status === 'ok'
                ? 'flex items-start gap-2 rounded-lg border border-positive/40 bg-positive/10 px-3 py-2 text-sm text-positive'
                : test.status === 'fail'
                  ? 'flex items-start gap-2 rounded-lg border border-negative/40 bg-negative/10 px-3 py-2 text-sm text-negative'
          ...
          <div
            className={
              brainStatus.type === 'success'
                ? 'flex items-center gap-2 rounded-lg border border-positive/40 bg-positive/10 px-3 py-2 text-sm text-positive'
                : brainStatus.type === 'error'
                  ? 'flex items-center gap-2 rounded-lg border border-negative/40 bg-negative/10 px-3 py-2 text-sm text-negative'
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <div className="flex items-center gap-2 font-semibold text-content">
                  ...
                  <span className="chip border-positive/40 bg-positive/10 text-positive text-xs">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <div className="font-semibold text-content flex items-center gap-2">
                  <span>Demo Mode</span>
                  <span className="chip border-neutral/40 bg-neutral/10 text-neutral text-xs">
                    {remainingDemoQueries} AI {remainingDemoQueries === 1 ? 'query' : 'queries'} left
                  </span>
                </div>
                <p className="text-xs text-muted mt-0.5">
                  Pay-what-you-want Paddle license (as little as $1) tied to your Google Account
                </p>
              </div>
              <div className="flex items-center gap-2">
                <button
                ...
                >
                  <BrandMark size="sm" className="brightness-0 invert" />
                  <span>Upgrade to Pro — as little as $1</span>
                </button>
                <button
                  type="button"
