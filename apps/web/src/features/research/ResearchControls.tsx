...
export function ReportButton({
  ...
  const run = () => {
    if (!subjectId) return;
    const taskId = taskManager.startTask('report', `Generating ${kind} report`);
    generateReport.mutate(
      { kind, subjectId, focus: focus.trim() || null },
      {
        onSuccess: (r) => {
          taskManager.completeTask(taskId, { message: `Report ready: ${r.title}` });
          setOpen(false);
          setFocus('');
          navigate(`/reports/${r.id}`);
        },
        onError: (error) => {
          taskManager.failTask(taskId, error instanceof Error ? error.message : 'Report generation failed.');
        },
      },
    );
  };
