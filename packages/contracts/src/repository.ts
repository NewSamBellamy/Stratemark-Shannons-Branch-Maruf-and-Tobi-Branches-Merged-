...
/** A current market signal fetched by the fast, separate updates search. */
...
export interface MarketUpdatesInput {
  prompt: string;
  region: string | null;
  limit?: number;
}
...
export interface MarketIntelRepository {
  ...
  /** Optional fast, separate grounded search for current market updates. */
  getMarketUpdates?(input: MarketUpdatesInput, handlers?: ResearchHandlers): Promise<MarketUpdate[]>;
