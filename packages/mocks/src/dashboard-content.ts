...
function teamOrg(ctx: ContentContext): TeamOrgContent {
  ...
  const nodes: TeamOrgContent['nodes'] = [
    { id: `${seed.slug}-ceo`, name: 'Jordan Rivera', role: 'Founder & CEO', group: 'exec', parentId: null, bio: 'Fixture bio: founded the label after a decade in apparel sourcing; the public face of the brand.', tenure: 'Founded the company in 2019', priorCompany: 'Unknown', notableProject: 'Company founding and brand direction' },
    { id: `${seed.slug}-coo`, name: 'Sam Okafor', role: 'COO', group: 'exec', parentId: `${seed.slug}-ceo`, bio: 'Fixture bio: runs operations and the wholesale channel.', tenure: null, priorCompany: null, notableProject: 'Operations and wholesale channel' },
    { id: `${seed.slug}-cpo`, name: 'Alex Nguyen', role: 'Head of Product', group: 'product', parentId: `${seed.slug}-ceo`, bio: 'Fixture bio: owns the product line and drop calendar.', tenure: null, priorCompany: null, notableProject: 'Product line and drop calendar' },
    { id: `${seed.slug}-design`, name: 'Riley Brooks', role: 'Creative Director', group: 'design', parentId: `${seed.slug}-cpo`, bio: '', tenure: null, priorCompany: null, notableProject: null },
  ];
  if (big) {
    nodes.push(
      { id: `${seed.slug}-cto`, name: 'Priya Shah', role: 'CTO', group: 'exec', parentId: `${seed.slug}-ceo`, bio: 'Fixture bio: leads e-commerce engineering.', tenure: null, priorCompany: null, notableProject: 'E-commerce engineering' },
      { id: `${seed.slug}-ai`, name: 'Marcus Lee', role: 'Head of AI', group: 'ai', parentId: `${seed.slug}-cto`, bio: '', tenure: null, priorCompany: null, notableProject: null },
      { id: `${seed.slug}-data`, name: 'Dana Cruz', role: 'ML Engineer', group: 'ai', parentId: `${seed.slug}-ai`, bio: '', tenure: null, priorCompany: null, notableProject: null },
    );
  }
  return { nodes };
}
