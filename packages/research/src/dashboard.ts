  ...
  switch (tab) {
    ...
    case 'team_org': {
      ...
      const out = await client.structure(
        `Convert to JSON { "nodes": [ { "id" (short slug), "name", "role", "group": "exec"|"ai"|"product"|"design"|"other", "parentId" (id of manager or null), "bio" (1-2 reported sentences, or "" when the notes say nothing about the person), "tenure" (reported joining year/date or duration, or null), "priorCompany" (a reported prior company, or null), "notableProject" (a reported project or ownership area, or null) } ] }. The top leader has parentId null. Only include tenure, priorCompany, or notableProject when the notes explicitly support them; otherwise use null. Never invent a bio or fill unknown fields from inference.\n\nNOTES:\n${g.text}`,
        teamOrgContentSchema,
        structSys,
      );
