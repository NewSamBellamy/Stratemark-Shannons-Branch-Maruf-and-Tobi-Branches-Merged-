...
export class GeminiRepository implements MarketIntelRepository {
  ...
  async getMarketUpdates(
    input: MarketUpdatesInput,
    handlers?: ResearchHandlers,
  ): Promise<MarketUpdate[]> {
    const limit = Math.min(10, Math.max(6, input.limit ?? 8));
    handlers?.onProgress?.({ message: 'Fetching a separate market-updates pass…', kind: 'step', taskId: handlers.taskId });
    const grounded = await this.client.ground(
      `Find the latest credible market updates for: ${input.prompt}${input.region ? ` in ${input.region}` : ''}. Search recent news, company announcements, product launches, funding, regulation, and relevant public trend signals. Return only developments that are timely and useful to someone evaluating this market. Do not repeat generic research methodology. Cite every item.`,
      { system: 'You are a market-news scout. Prefer recent, attributable developments and clearly distinguish a reported event from analysis. Never invent a headline or date.', signal: handlers?.signal },
    );
    ...
    const structured = await this.client.structure(
      `Using only the grounded source text and citations below, select up to ${limit} distinct recent market updates. Do not echo search-process commentary. Each summary must explain why the development matters to this market.\n\nGROUNDED TEXT:\n${grounded.text}\n\nCITATIONS:\n${grounded.citations.map((c, i) => `[${i}] ${c.title} — ${c.url}`).join('\n')}`,
      updateSchema,
      { system: 'Output only a JSON array. Every citationIndex must refer to a provided citation. If evidence is insufficient, return fewer items.', signal: handlers?.signal },
    );
    return structured.flatMap((item, index) => {
      const citation = grounded.citations[item.citationIndex];
      if (!citation) return [];
      handlers?.onProgress?.({ message: `Update ready: ${item.headline}`, kind: 'find', taskId: handlers.taskId });
      return [{
        id: `update_${Date.now().toString(36)}_${index}`,
